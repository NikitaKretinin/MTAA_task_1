...
#    Adapted code from https://github.com/tirfil/PySipFullProxy
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or origin not in registrar:
            self.sendResponse("400 Zla Ziadost")
            return
        destination = self.getDestination()
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                text = text.replace("Ok", "VYBAVENE OK", 1)
                text = text.replace("Busy Here", "Obsadene", 1)
                socket.sendto(text.encode("utf-8"),claddr)
                showtime()
